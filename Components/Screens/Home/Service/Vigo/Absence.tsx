      ...
      {absenceData.map((item, index) => (
        <View
          ...
          style={{
            ...
            backgroundColor: limitReached(item.absence)
              ? "rgba(240,128,128,0.76)"
              : "transparent",
          }}
        >
          <Text style={styles.textAbsence} allowFontScaling={true}>
            {item.subject}
          </Text>
          <Text style={styles.textAbsence} allowFontScaling={true}>
            {Math.ceil(item.absence * 100) + "%"}
          </Text>
        </View>
      ))}
    </View>
